...
func TestReset(t *testing.T) {
	...
	t.Run("Happy path", func(t *testing.T) {
		...
		err = Reset(t.Context(), w.Workspace, r.Repository, wstd.ResetOptions(remoteRev1, false))
		assert.NoError(err)
		...
		err = Reset(t.Context(), w.Workspace, r.Repository, wstd.ResetOptions(remoteRev2, false))
		assert.NoError(err)
	...
	t.Run("Handle local changes (fail or force)", func(t *testing.T) {
		...
		err = Reset(t.Context(), w.Workspace, r.Repository, wstd.ResetOptions(remoteRev1, true))
		assert.NoError(err)
		assert.Equal(remoteRev1, w.Head())
		assert.Equal([]lib.TestFileInfo{
			{"a.txt", 0o600, 1, "a"},
	...
	t.Run("With path-prefix", func(t *testing.T) {
		...
		// Reset to the first commit.
		err = Reset(t.Context(), prefixW.Workspace, r.Repository, wstd.ResetOptions(remoteRev1, false))
		assert.NoError(err)
		...
		err = Reset(t.Context(), prefixW.Workspace, r.Repository, wstd.ResetOptions(remoteRev2, false))
		assert.NoError(err)
	...
	t.Run("RestorableMetadata is respected", func(t *testing.T) {
		...
		err = Reset(t.Context(), w.Workspace, r.Repository, opts)
		assert.NoError(err)
