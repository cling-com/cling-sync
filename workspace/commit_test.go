...
func commitFilesDest(t *testing.T, r *lib.TestRepository) *CommitFilesDest {
	...
	assert := lib.NewAssert(t)
	head := r.Head()
	snapshot, err := lib.NewRevisionSnapshot(t.Context(), r.Repository, head, td.NewFS(t))
	assert.NoError(err)
...
func commitFilesSrc(t *testing.T, r *lib.TestRepository, src *lib.TestFS, prefix lib.Path) *CommitFilesSrc {
	...
	assert.NoError(err)
	snapshot, err := lib.NewRevisionSnapshot(t.Context(), r.Repository, r.Head(), td.NewFS(t))
	assert.NoError(err)
